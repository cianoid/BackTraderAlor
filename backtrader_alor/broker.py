...
class Broker(with_metaclass(MetaBroker, BrokerBase)):
            ...
            if portfolio and exchange:  # Если считаем свободные средства по портфелю/бирже
                cash = next(
                    (
                        ...
                        if key[0] == portfolio and key[1] == exchange and not key[2]
                    ),
                    None,
                )  # Денежная позиция по портфелю/рынку
            else:  # Если считаем свободные средства по всем счетам
                cash = sum(
                    [position.price for key, position in self.positions.items() if not key[2]]
                )  # Сумма всех денежных позиций
    ...
    def getvalue(self, datas=None, portfolio=None, exchange=None):
        ...
        if self.store.BrokerCls:  # Если брокер есть в хранилище
            if datas:  # Если считаем стоимость позиции/позиций
                data: Data  # Данные Алор
                for data in datas:  # Пробегаемся по всем тикерам
                    position = self.positions[
                        (data.portfolio, data.exchange, data.board, data.symbol)
                    ]  # Позиция по тикеру
                ...
                value = sum(
                    [
                        ...
                        for key, position in self.positions.items()
                        if key[0] == portfolio and key[1] == exchange and key[2]
                    ]
                )  # Стоимость позиций по портфелю/бирже
            else:  # Если считаем стоимость всех позиций
                value = sum(
                    [position.price * position.size for key, position in self.positions.items() if key[2]]
                )  # Стоимость всех позиций
                self.value = value  # Сохраняем текущую стоимость позиций
        return value

    def getposition(self, data: Data):
        """Позиция по тикеру
    ...
    def get_notification(self):
        return (
            self.notifs.popleft() if self.notifs else None
        )  # Удаляем и возвращаем крайний левый элемент списка уведомлений или ничего
    ...
    def is_subscribed(self, portfolio, exchange):
        ...
        for guid in self.store.provider.subscriptions.keys():  # Пробегаемся по всем подпискам
            ...
            if (
                "portfolio" not in subscription or "exchange" not in subscription
            ):  # Если подписка не по портфелю/бирже (например, на бары)
    ...
    def subscribe(self, portfolio, exchange):
        ...
        :param str exchange: Биржа 'MOEX' или 'SPBX'
        """
        self.store.provider.positions_get_and_subscribe_v2(
            portfolio, exchange
        )  # Подписка на позиции (получение свободных средств и стоимости позиций)
        self.store.provider.trades_get_and_subscribe_v2(
            portfolio, exchange
        )  # Подписка на сделки (изменение статусов заявок)
        self.store.provider.orders_get_and_subscribe_v2(
            portfolio, exchange
        )  # Подписка на заявки (снятие заявок с биржи)
        self.store.provider.stop_orders_get_and_subscribe_v2(
            portfolio, exchange
    ...
    def unsubscribe(self):
        ...
        subscriptions = (
            self.store.provider.subscriptions.copy()
        )  # Работаем с копией подписок, т.к. будем удалять элементы
        for guid, subscription_request in subscriptions.items():  # Пробегаемся по всем подпискам
            if subscription_request["opcode"] in (
                "PositionsGetAndSubscribeV2",  # Если это подписка на позиции (получение свободных средств и стоимости
                # позиций)
            ...
            for exchange in account["exchanges"]:  # Пробегаемся по всем биржам
                ...
                for position in positions:  # Пробегаемся по всем позициям
                    ...
                    if position["isCurrency"]:  # Если пришли валютные остатки (деньги)
                        ...
                        price = self.store.provider.alor_price_to_price(
                            exchange, symbol, position["avgPrice"]
                        )  # Цена входа
                        value += price * size  # Увеличиваем общий размер стоимости позиций
                    self.positions[(portfolio, exchange, board, symbol)] = Position(
                        size, price
    ...
    def get_order(self, order_number) -> Union[Order, None]:
        ...
        Пробегаемся по всем заявкам на бирже. Если нашли совпадение с номером заявки на бирже, то возвращаем заявку
        BackTrader. Иначе, ничего не найдено
    ...
    def create_order(
        ...
        **kwargs,
        ...
        if order.exectype in (
            ...
            self.logger.warning(
                f"Постановка заявки {order.ref} по тикеру {data.board}.{data.symbol} на бирже {data.exchange} "
        ...
        portfolio = (
            self.store.provider.get_account(data.board, order.info["account_id"])["portfolio"]
            if "account_id" in order.info
            else data.portfolio
        )  # Портфель из заявки/тикера
        ...
        if (
            ...
            self.logger.warning(
                f"Постановка заявки {order.ref} по тикеру {data.board}.{data.symbol} на бирже {data.exchange} "
                f"отклонена. {price_type} цена (price) не указана для заявки типа {order.exectype}"
            )
            order.reject(self)  # то отклоняем заявку
            self.oco_pc_check(order)  # Проверяем связанные и родительскую/дочерние заявки
            return order  # Возвращаем отклоненную заявку
        if (
            ...
            self.logger.warning(
                f"Постановка заявки {order.ref} по тикеру {data.board}.{data.symbol} на бирже {data.exchange} "
                f"отклонена. Лимитная цена (pricelimit) не указана для заявки типа {order.exectype}"
            )
            order.reject(self)  # то отклоняем заявку
            self.oco_pc_check(order)  # Проверяем связанные и родительскую/дочерние заявки
            return order  # Возвращаем отклоненную заявку
        ...
        if not transmit or parent:  # Для родительской/дочерних заявок
            ...
            if order.ref != parent_ref and parent_ref not in self.pcs:
                ...
                self.logger.warning(
                    f"Постановка заявки {order.ref} по тикеру {data.board}.{data.symbol} на бирже {data.exchange} "
                    f"отклонена. Родительская заявка не найдена"
                )
                order.reject(self)  # то отклоняем заявку
                self.oco_pc_check(order)  # Проверяем связанные и родительскую/дочерние заявки
                return order  # Возвращаем отклоненную заявку
        ...
        # то возвращаем созданную заявку со статусом Created. На биржу ее пока не отправляем
        return order
    ...
    def place_order(self, order: Order):
        ...
        if order.exectype == Order.Market:  # Рыночная заявка
            ...
            response = self.store.provider.create_stop_order(
                portfolio, exchange, symbol, class_code, side, quantity, stop_price, condition
            )
            ...
            condition = "MoreOrEqual" if order.isbuy() else "LessOrEqual"  # Условие срабатывания стоп цены
            response = self.store.provider.create_stop_limit_order(
                portfolio, exchange, symbol, class_code, side, quantity, stop_price, limit_price, condition
            )
        ...
        if not response:  # Если при отправке заявки на биржу произошла веб ошибка
            ...
            self.logger.warning(
                f"Постановка заявки по тикеру {board}.{symbol} на бирже {exchange} отклонена. Ошибка веб сервиса"
            )
            order.reject(self)  # то отклоняем заявку
            self.oco_pc_check(order)  # Проверяем связанные и родительскую/дочерние заявки
            return order  # Возвращаем отклоненную заявку
    ...
    def cancel_order(self, order) -> Order | None:
        ...
        if not order.alive():  # Если заявка уже была завершена
            return  # noqa:R502 то выходим, дальше не продолжаем
