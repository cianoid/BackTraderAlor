...
class Store(with_metaclass(MetaSingleton, object)):
    ...
    logger = logging.getLogger("Store")  # Будем вести лог
    ...
    def start(self):
        self.provider.on_new_bar = lambda response: self.new_bars.append(
            dict(guid=response["guid"], data=response["data"])
        )  # Обработчик новых баров по подписке из Алор
        # События WebSocket Thread/Task для понимания, что происходит с провайдером
        self.provider.on_entering = lambda: self.logger.info("WebSocket Thread: Запуск")
        self.provider.on_enter = lambda: self.logger.info("WebSocket Thread: Запущен")
        self.provider.on_connect = lambda: self.logger.info("WebSocket Task: Подключен к серверу")
        self.provider.on_resubscribe = lambda: self.logger.info(
            f"WebSocket Task: Возобновление подписок ({len(self.provider.subscriptions)})"
        )
        self.provider.on_ready = lambda: self.logger.info("WebSocket Task: Готов")
        self.provider.on_disconnect = lambda: self.logger.info("WebSocket Task: Отключен от сервера")
        self.provider.on_timeout = lambda: self.logger.info("WebSocket Task: Таймаут")
        self.provider.on_error = lambda response: self.logger.info(f"WebSocket Task: {response}")
        self.provider.on_cancel = lambda: self.logger.info("WebSocket Task: Отмена")
        self.provider.on_exit = lambda: self.logger.info("WebSocket Thread: Завершение")
